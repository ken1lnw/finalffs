...
export default function DeleteButton(props:any) {
  ...
  const handlePrint = async () => {
    ...
    try {
      const deleteuser = await fetch(`/api/dbuser/${props.userId}`, {
          method: "DELETE",
          body: JSON.stringify({}),
          headers: {
      ...
      if (deleteuser.ok) {
          ...
          console.error("User Delete request failed");
      }
      props.refreshData();
      setOpen(false);
  } catch (error) {
                      ...
                      <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        <div className="mt-2">
                          <p className="text-sm text-gray-500">
                            กรุณาตรวจสอบให้แน่ใจว่าต้องการลบผู้ใช้งาน เนื่องจากจะไม่สามารถกู้คืนร้องที่ลบไปแล้วได้
                          </p>
                        </div>
                      </div>
                    </div>
